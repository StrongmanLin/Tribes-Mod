...
public class Tirek extends EntityMob implements IBossDisplayData, IRangedAttackMob
{
	...
	private static final IEntitySelector attackEntitySelector = new EntityTirekAttackFilter();
	
	public Tirek(World par1World)
	{
		super(par1World);
		this.setHealth(2000);
    ...
    protected void updateAITasks()
    {
        ...
        if (this.func_82212_n() > 0)
        {
            ...
            if (i <= 0)
            {
                this.worldObj.newExplosion(this, this.posX, this.posY + (double)this.getEyeHeight(), this.posZ, 100.0F, false, this.worldObj.getGameRules().getGameRuleBooleanValue("mobGriefing"));
                            ...
                            if (entitylivingbase != this && entitylivingbase.isEntityAlive() && this.canEntityBeSeen(entitylivingbase))
                            {
                                if (entitylivingbase instanceof EntityPlayer)
                                {
                                    if (!((EntityPlayer)entitylivingbase).capabilities.disableDamage)
                                    {
                                        ...
                                        {
                                        	attackEntityWithRangedAttack(entitylivingbase, 0);
                                        }
                                        else
                                        {
                                        	float f1 = entitylivingbase.getDistanceToEntity(this);
                                        	this.attackEntity(entitylivingbase, f1);
                                        }
    ...
    private void func_82209_a(int par1, double par2, double par4, double par6, boolean par8)
    {
        ...
        entitylargefireball.posY = d4;
        entitylargefireball.posX = d3;
        entitylargefireball.posZ = d5;
    ...
    {
        this.dropItem(TribesMod.elementsOfHarm.itemID, 1);
    }
